...
### 13.3 Audit-pass-5 (2026-07-27) — report §7 (the eight specialization classes)

**§7 audits clean.** One stale line citation, no incorrect data. This is the
best-evidenced chapter in the report and the reason is structural: every number
in it comes from a committed harness over committed artifacts, so the audit is a
rebuild rather than an archaeology.

| check | result |
|---|---|
| all 8×7 cells of the headline table vs `spec_examples/stats.csv` | exact |
| `stats.csv` regenerated by `build_examples.sh` | byte-identical |
| all 16 `.s` files rebuilt | byte-identical |
| independent recount of instrs/VALU/VGPR/SGPR/`s_load`/`ds`/branch from the `.s` | matches CSV on every cell |
| 8 `v2_ops_body.inc` line citations | all land on the named function |
| `v2_ops.h:222-223` (reduction-order constraint) | **stale — the comment is at `235-236`**; `222-223` is `scatter_bits_2`. Quote itself verbatim. Corrected in the chapter. |
| `circuit_d5` opcode-mix table (7 families, counts + shares) | exact, all 7 rows; total 1,720 matches the `coop_r10_n1720` kernel name |
| S1: 4 `v_readfirstlane_b32` in interp, 0 in spec | exact |
| S1: literal masks `8`/`0x400`/`0x1000`/`64` | exact, and those are the only ones |
| S1: "the 8 surviving VALU are just `v_mov`s" | exact — all 8 are `v_mov_b32`/`v_mov_b64` staging scalars for `global_store` |
| S2: 18 branches → 0 | exact |
| S3: exactly one `s_load ... 0x278` in interp, none in spec | exact |
| S3: `0x278` is `active_k`'s offset | confirmed independently — `8·5 + 8·5 + 8·64 + 8·4 + 8 = 632 = 0x278` |
| S4: `ds_*` 4→4 (the ABI-invariance evidence) | exact |
| S5: `s_load` 4→1, survivors identified as `0x20`/`0x18`/`0x0`/`0x278` | exact, in the stated file order |
| S5: `offset:288` vs `offset:32` for `t_bit = 1<<5` at 8 B/amplitude | exact |
| S6: 2 surviving branches, "both `s_cbranch_execz`" | exact |
| S7: range-test idiom `0xffffff7d` = −131 = −`start`, compared against `56` = `count`−1 | exact |
| S7: VGPR 56→56, SGPR 86→85 from the `.set` literals | exact |
| every quoted assembly block, against its stated line range | verbatim |
| every quoted C source block and case-file comment | verbatim |

**Fifth method note: build the artifact into the claim.** §7's ratios are the
most aggressive numbers in the report (up to 5.50×) and were the fastest to
verify — under a minute for the whole chapter — because `build_examples.sh`
regenerates the CSV and all sixteen `.s` files byte-for-byte. Contrast §11.2 and
§12.2, where a single uncommitted script cost hours each and one of them was
wrong. The distinguishing property is not that §7 was measured more carefully;
it is that its measurement is *executable*.

Note also that §7 documents its own prior defects in-chapter (the ISA-line-count
dilution that understated S1 as 1.37× rather than 2.89×, and the symbolic
`; NumVgprs:` scrape that made S7 read as 0 registers). Those were caught and
fixed before this audit, by the same mechanism — the harness comment in
`build_examples.sh:26-40` explains exactly why the `.set` directives are parsed
